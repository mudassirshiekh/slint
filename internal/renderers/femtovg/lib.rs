...
mod itemrenderer;
mod opengl;
pub use opengl::OpenGLInterface;
mod wgpu;
        ...
        window_inner
            .draw_contents(|components| -> Result<(), PlatformError> {
                ...
                if let Some(notifier_fn) = self.rendering_notifier.borrow_mut().as_mut() {
                    ...
                    femtovg_canvas.flush_to_surface(surface.render_surface());
                ...
                canvas.borrow_mut().flush_to_surface(surface.render_surface());
...
pub struct FemtoVGOpenGLRenderer {
    inner: FemtoVGRenderer<opengl::OpenGLBackend>,
}

impl FemtoVGOpenGLRenderer {
...
#[doc(hidden)]
pub trait FemtoVGRendererExt<B: GraphicsBackend> {
...
#[doc(hidden)]
impl<B: GraphicsBackend> FemtoVGRendererExt<B> for FemtoVGRenderer<B> {
...
#[doc(hidden)]
impl FemtoVGRendererExt<OpenGLBackend> for FemtoVGOpenGLRenderer {
    fn new_without_context() -> Self {
        Self { inner: FemtoVGRenderer::new_without_context() }
    }

    fn clear_graphics_context(&self) -> Result<(), i_slint_core::platform::PlatformError> {
        self.inner.clear_graphics_context()
    }
    ...
    fn render_transformed_with_post_callback(
        ...
        self.inner.render_transformed_with_post_callback(
            rotation_angle_degrees,
            translation,
            surface_size,
            post_render_cb,
    ...
    fn backend(&self) -> &OpenGLBackend {
        self.inner.backend()
    }
}

impl RendererSealed for FemtoVGOpenGLRenderer {
    fn free_graphics_resources(
        &self,
        component: i_slint_core::item_tree::ItemTreeRef,
    ...
    fn set_rendering_notifier(
        ...
        self.inner.set_rendering_notifier(callback)
    }
    ...
    fn text_size(
        ...
        self.inner.text_size(font_request, text, max_width, scale_factor, text_wrap)
    }
    ...
    fn font_metrics(
        ...
        self.inner.font_metrics(font_request, scale_factor)
    }
    ...
    fn text_input_byte_offset_for_position(
        ...
        self.inner.text_input_byte_offset_for_position(text_input, pos, font_request, scale_factor)
    }
...
#[doc(hidden)]
impl FemtoVGOpenGLRendererExt for FemtoVGOpenGLRenderer {
    fn set_opengl_context(
        &self,
        #[cfg(not(target_arch = "wasm32"))] opengl_context: impl OpenGLInterface + 'static,
        #[cfg(target_arch = "wasm32")] html_canvas: web_sys::HtmlCanvasElement,
    ) -> Result<(), i_slint_core::platform::PlatformError> {
        self.inner.graphics_backend.set_opengl_context(
            &self.inner,
            #[cfg(not(target_arch = "wasm32"))]
            opengl_context,
            #[cfg(target_arch = "wasm32")]
